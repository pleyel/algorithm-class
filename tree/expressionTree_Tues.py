...
def encode(ch):
    ...
    if ch == ' ':
        ...
        return MORSE_TABLE[idx][1] # 해당 문자의 모스코드 변환
    else:
        return '?' # 미지원 문자
    
# 3. 디코드 : 결정트리로 모스코드 -> 해당 문자열로 변환
def decode(root, code):
    # 선형구조 대신 계층구조인 트리 사용 : 효율적 방식
    # 파라미터 : root는 결정트리
    if code == '/':
        return ' '
    node = root # 결정트리의 루트 시작
    for c in code: # 모스코드 문자열(., -)에 대하여 반복 처리
        if c == '.': # 점이먼
...
def make_morse_tree():
    ...
    for ch, code in MORSE_TABLE: # 모스코드 테이블의 각 튜플(문자, 부호)에 대해 반복 처리
        cur= root # 로트노드부터 시작
        for c in code:
            if c == '.':
                if cur.left is None:
                ...
                cur = cur.right # 왼쪽 자식 노드로 이동
        # 부호의 끝에 도달했으면 해당 노드에 문자 저장
        cur.data = ch
    return root # 결정트리의 루트노드 반환
