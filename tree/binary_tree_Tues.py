...
class BTNode:
	...
	def __repr__(self): # 트리의 노드를 문자열 표현 - print 함수 호출
		return f"BTNode({self.data !r}, {self.left}, {self.right})"
...
def is_leaf(node): # 노드가 None이 아니고, 왼쪽과 오른쪽 자식 노드가 모두 None인 경우(단일노드)
	if node is None:
		return False
	return node.left is None and node.right is None # leaf node -> True 반환

def print_leaf_nodes(node):	# 이진트리의 단말노드만 출력 - 재귀적 정의
	if node is None: # 재귀호출 종료 조건
		return # 종료
	if is_leaf(node): # 리프노드이면 재귀호출 종료 조건
		print(f"{node.data} is a leaf node") # 단말 노드 출력
		return # 종료
	# 리프노드가 아니면 재귀호출
...
def inorder(node):	# 중위순회 : 왼쪽 -> 현재노드 -> 오른쪽 순회 : 재귀적 정의
	if node is None: # 재귀호출 종료
		return
	inorder(node.left)
	print_data(node.data)
...
def postorder(node):	# 후위순회 : 왼쪽 -> 오른쪽 -> 현재노드 순회 : 재귀적 정의
	if node is None: # 재귀호출 종료
		return
	postorder(node.left)
...
def levelorder(root):	# 레벨순회 : 레벨 순으로 노드 방문 (루트 레벨 -1, 아래 내려갈수록 레벨이 증가)
	# 파이썬 모듈 큐 이용
	...
	while q: # 큐가 공백이 될때까지 반복 처리
		node = q.popleft()
		print_data(node.data) # 트리의 노드 방문(출력) -> 큐애서 나오는 순서가 방문 순서 결정
...
def count_nodes(root): # 이진트리의 총 노드의 개수
	...
	if root is None: # 재귀호출 종료 조건
		return 0
	return count_nodes(root.left) + count_nodes(root.right) + 1

def tree_height(root): # 이진트리의 높이 계산
	# 재귀적 정의 : 트리가 비어 있으면 0, 아니면 왼쪽과 오른쪽 서브트리의 높이 중 큰 값 + 1
	if root is None: # 재귀호출 종료 조건
		return 0
...
def bitree_to_array(root): # 이진트리 -> 배열(리스트)로 레벨 순서대로 반환
	...
	while queue:
		...
		if node:
			...
			queue.append(node.left) # None도 추가하여 위치 유지
			queue.append(node.right)
		else:
			result.append(None)
	return result

def full_binary_tree_nodes(k): # 높이가 k인 포화이진트리의 노드 개수 구하기 (루트레벨 - 1)
	return 2**k - 1

def min_nodes_in_binary_tree(k): # 높이가 k인 이진트리의 최소 노드 개수 구하기 (루트레벨 -1)
...
def max_nodes_in_binary_tree(k): # 높이가 k인 이진트리의 최대 노드 개수 구하기 (루트레벨 -1)
	return full_binary_tree_nodes(k)
	
def count_none_links(root): # 이진트리에서 노드의 개수 N이라면 연결되지 않은 링크 수 구하기
	# 재귀적 정의 : 노드가 None이면 1, 아니면 왼족과 오른족 서브트리의 연결되지 않은 링크 수 합
	if root is None: # 재귀호출의 종료 조건
		return 1
