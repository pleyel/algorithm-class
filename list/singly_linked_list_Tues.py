...
3. append(node): 현재 노드 뒤에 주어진 새로운 노드(new)를 연결
...
class LinkedList:
    ...
    def insert(self, pos, elem):
        ...
        if pos < 0 : return None # 자동으로 유효하지 않는 위치에 대해 ValueError 발생

        new = Node(elem) # 또는 None(elem, None) # 1. 새 노드 생성
        before = self.getNode(pos-1) # 2. pos - 1 위치에 노드를 가져오기
    ...
    def delete(self, pos):
        ...
        if pos < 0:
            raise IndexError("empty 또는 범위 밖")
        
        before = self.getNode(pos-1) # 1. 삭제할 노드의 이전 노드
